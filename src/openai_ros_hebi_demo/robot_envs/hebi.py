...
class HebiEnv(robot_gazebo_env.RobotGazeboEnv):

    def __init__(self, ros_ws_abspath):
        rospy.logdebug("========= In Hebi Env")

        # We launch the ROSlaunch that spawns the robot into the world
        # ROSLauncher(rospackage_name="hebi_gazebo",
        #             launch_file_name="arm_moveit_simulation.launch",
    ...
    def set_trajectory_ee(self, action):
        ...
        result = self.move_hebi_object.ee_traj(ee_target)
        return result
    ...
    def set_trajectory_joints(self, initial_qpos):
        ...
        self.move_hebi_object.joint_traj(positions_array)
    ...
    def get_ee_pose(self):
        ...
        """
        self.gazebo.unpauseSim()
        gripper_pose = self.move_hebi_object.ee_pose()
        self.gazebo.pauseSim()
        return gripper_pose

    def get_ee_rpy(self):
        gripper_rpy = self.move_hebi_object.ee_rpy()
        return gripper_rpy

    def wait_hebi_ready(self):
        """
...
class Obj_Pos(object):
    ...
    def __init__(self):
        ...
        world_specs = rospy.ServiceProxy('/gazebo/get_world_properties', 
            GetWorldProperties)()
        self.time = 0
...
class MoveHebi(object):
    def __init__(self):
        rospy.logdebug("===== In MoveHebi")
        moveit_commander.roscpp_initialize(sys.argv)
        self.robot = moveit_commander.RobotCommander()
        self.scene = moveit_commander.PlanningSceneInterface()
        self.group = moveit_commander.MoveGroupCommander("hebi_arm")
        rospy.logdebug("===== Out MoveHebi")
